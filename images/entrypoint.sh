...
# MULTUS_LOG_FILE="/etc/cni/net.d/multus.d/multus.log"
...
while [ "$1" != "" ]; do
    ...
    case $PARAM in
        ...
        --namespace-isolation)
            ...
            ;;
        --multus-log-level)
            # log "!bang WARNING: Log level FORCED for debugging (remove this)"
...
EOF
      ...
      # log "Original file -------------------------------------------"
      # echo $(cat ${MULTUS_AUTOCONF_DIR}/${MASTER_PLUGIN}.old)
      # echo ""
      # log "Generated file -------------------------------------------"
      # echo $(cat $CNI_CONF_DIR/00-multus.conf)
      # echo ""
      # log "CNI_CONF_DIR contents -------------------------------------------"
      # echo $(ls -lath $CNI_CONF_DIR)
      # echo ""
    fi
  done
fi
...
while true; do
  if [ "$MULTUS_CLEANUP_CONFIG_ON_EXIT" == true ]; then
    ...
    if [ ! -f "$MASTER_PLUGIN_LOCATION" ]; then
      ...
      cleanup
      generateMultusConf
      log "Entering watch loop after regeneration..."
    fi
    sleep 1
  else
